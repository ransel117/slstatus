...
static const char *pick(const char *bat, const char *f1, const char *f2, char *path, size_t length) {
    if (esnprintf(path, length, f1, bat) > 0 && access(path, R_OK) == 0) return f1;
    if (esnprintf(path, length, f2, bat) > 0 && access(path, R_OK) == 0) return f2;
...
const char *battery_perc(const char *bat) {
    ...
    if (esnprintf(path, sizeof(path), POWER_SUPPLY_CAPACITY, bat) < 0) return NULL;
    if (pscanf(path, "%d", &cap_perc) != 1) return NULL;
...
const char *battery_state(const char *bat) {
    ...
    if (esnprintf(path, sizeof(path), POWER_SUPPLY_STATUS, bat) < 0) return NULL;
    if (pscanf(path, "%12[a-zA-Z ]", state) != 1) return NULL;
...
const char *battery_perc(const char *unused) {
    ...
    if (load_apm_power_info(&apm_info)) return bprintf("%d", apm_info.battery_life);
