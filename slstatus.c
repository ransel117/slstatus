...
int main(int argc, char *argv[]) {
    ...
    if (!sflag && !dpy) die("XOpenDisplay: Failed to open display");
    ...
    while (!done) {
        ...
        for (i = len = 0; i < LEN(args); i++) {
            ...
            if ((ret = esnprintf(status + len, sizeof(status) - len, args[i].fmt, res)) < 0) break;
